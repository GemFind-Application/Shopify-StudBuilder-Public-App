...
class AppUninstalledJob extends \Osiset\ShopifyApp\Messaging\Jobs\AppUninstalledJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        $charge = DB::table('charges')->where('user_id',$userId)->first();
        $hostname = $finalData['name'];
        $apppassword = $shop['password'];
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER,array('Content-Type:application/json'));
		$url = 'https://'.$apiKey.':'.$apppassword.'@'.$hostname.'/admin/api/'.env('SHOPIFY_API_VERSION').'/recurring_application_charges/'.$charge['charge_id'].'.json';
        curl_setopt($ch, CURLOPT_URL,$url);
        $server_output = curl_exec($ch);
        $charge_delete = json_decode($server_output,true);
